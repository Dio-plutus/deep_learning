...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train):
        dists[i] = torch.sum((y - x[i])**2, dim=1).t()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  dists = -2 * (x @ y.t()) + torch.sum(y**2, dim=1) + torch.sum(x**2, dim=1).reshape(-1, 1)

  #-2 * (x @ y.t()) + torch.sum(y ** 2, dim=1) + torch.sum(x ** 2, dim=1).reshape(-1, 1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
